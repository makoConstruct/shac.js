...
(function() {
  var arrayRemoveElement, elBounds,
  ...
  this.attachAutocompletion = function(input, matchsetArray, matchCallback) {
    ...
    config.nresults || (config.nresults = 10);
    config.matchset || (config.matchset = new this.MatchSet(config.matchsetArray, config.matchingLetterCSSClass));
    ac = new this.Autocompletion(config.matchset, config.nresults, config.dropdownCSSClass, config.matchCSSClass);
    ac.attach(input);
  ...
  this.Autocompletion = (function() {
    function Autocompletion(matchset, nresults, dropClass, matchClass) {
    ...
    Autocompletion.prototype.deploy = function() {
      ...
      if (this.dropListContainer.children.length > 0) {
        ep = elBounds(this.input);
        this.drop.style.left = ep.x + 'px';
        this.drop.style.top = ep.y + ep.h + 'px';
    ...
    Autocompletion.prototype.onInput = function() {
      var res;
      res = this.matchset.seek(this.input.value, this.nresults);
      this.pushToDrop(res);
      if (res.length > 0) {
        return this.deploy();
      }
